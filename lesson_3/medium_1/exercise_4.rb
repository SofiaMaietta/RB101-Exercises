...
# Yes, the difference is that in the first method, she uses the shovel operator << which is also a mutating method. Therefore, the object that buffer references will be mutated each time a new element is appended.

# However, with the second method, she uses the + operator to append a new element onto her array. Because this is not a mutating operation, a new object is created and returned each time, so the object that input_array references is never mutated.
